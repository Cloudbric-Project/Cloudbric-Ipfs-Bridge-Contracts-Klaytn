...
const dataStorage = `${__dirname}/../../data/threatdb/phishing_url`;
...
async function preprocess() {
    ...
    try {
        ...
        const getLastIdxQuery = "SELECT idx FROM phishing_url_uploaded_log ORDER BY idx DESC LIMIT 1";
    ...
    const fetchQueryFromStartingIdx = 
        `SELECT * FROM td_phishing_url_detail \
        WHERE idx >= ${startIdx} \
        ORDER BY idx ASC \
        LIMIT ${size}`;
